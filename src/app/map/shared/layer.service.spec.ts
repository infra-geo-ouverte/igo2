  ...
  it('create WFS layer with style ...', inject([LayerService], (service: LayerService) => {
    ...
    service.createLayer(layerOptions).subscribe(
      layer => {
        ...
        const fctStyle = (layer.olLayer as any).getStyle();
        type Polygon = [number, number][][];
        const polygon: Polygon = [[[90, 90], [90, 0], [0, 0], [0, 90], [90, 90]]];
        const feat: any = new ol.Feature({
          geometry: new ol.geom.Polygon(polygon),
          idFeat: '666Test'
        });
        feat.setStyle(fctStyle);
        const styleFeat = feat.getStyle();
  ...
  it('create WFS layer with Circle ...', inject([LayerService], (service: LayerService) => {
    ...
    service.createLayer(layerOptions).subscribe(
      layer => {
        ...
        const fctStyle = (layer.olLayer as any).getStyle();
        const point: [number, number] = [90, 90];
        const feat = new ol.Feature({
          geometry: new ol.geom.Point(point),
          idFeat: '666Test'
        });
        feat.setStyle(fctStyle);
        const styleFeat: any = feat.getStyle();
